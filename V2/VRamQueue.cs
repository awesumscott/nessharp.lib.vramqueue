...
public class VRamQueue : Module {
	...
	public void Push(VByte val) => _liveQueue.Push(val);
	public void Push(Func<RegisterA> val) => _liveQueue.Push(val.Invoke());
	public void DonePushing() => _liveQueue.PushDone();
	//TODO: remove this and use the op directly
	public void EndFrame() => _liveQueue.PushOnce(Y, Op<EOF>().Id);
	...
	public void Execute() {
		Action loopBody = () => {
			...
			AL.Comment("Use current Op to find the op handler address, then indirect JMP");
	...
	[CodeSection]
	private void Handler_NOP() => AL.GoTo(_executeLoopContinue);

	public void Add(Action codeSection) => _opHandlers.Add(AL.LabelFor(codeSection));

	public void Break() => AL.GoTo(_executeLoopBreak);
	public void Continue() => AL.GoTo(_executeLoopContinue);

...
public class VRamQueue : Module {
	...
	public void SetupAligned(Address pageStart, U8 length, params Option[] options) {
		...
		var VRAM = Ram.Allocate(pageStart, AL.Addr((U16)(pageStart + 0xFF)), "VRAM");
	...
	public void Push(IOperand val) => _liveQueue.Push(val);
	...
	public void EndFrameROM() => AL.Raw(Op.EOF);
	public void NOPROM() => AL.Raw(Op.NOP);
	...
	public void CopyBlock(Action action) => _liveQueue.PushRangeOnce(action);
